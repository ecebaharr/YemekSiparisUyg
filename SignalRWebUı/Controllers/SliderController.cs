...
namespace SignalRWebUI.Controllers
{
    public class SliderController : Controller
    {
        ...
        public async Task<IActionResult> CreateSlider(CreateSliderDto createSliderDto)
        {
            ...
            var responseMessage = await client.PostAsync("http://localhost:5044/api/Slider/", stringContent);
            if (responseMessage.IsSuccessStatusCode)
            {
        ...
        {
            var client = _httpClientFactory.CreateClient();
            var responseMessage = await client.DeleteAsync($"http://localhost:5044/api/Slider/{id}");
            if (responseMessage.IsSuccessStatusCode)
            {
                return RedirectToAction("Index");
            }
        ...
        {
            var client = _httpClientFactory.CreateClient();
            var responseMessage = await client.GetAsync($"http://localhost:5044/api/Slider/{id}");
            if (responseMessage.IsSuccessStatusCode)
            {
                var jsonData = await responseMessage.Content.ReadAsStringAsync();
