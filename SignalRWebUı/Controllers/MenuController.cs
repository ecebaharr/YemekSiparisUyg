...
namespace SignalRWebUı.Controllers
{
    public class MenuController : Controller
    {
        ...
        public async Task<IActionResult> AddBasket(int id, int menuTableId)
        {
            if (menuTableId == 0)
            {
                return BadRequest("Masa no 0 geliyor.");
            }
            ...
            var responseMessage = await client.PostAsync("http://localhost:5044/api/Basket", stringContent);

            var client2 = _httpClientFactory.CreateClient();
            //var jsonData2 = JsonConvert.SerializeObject(updateCategoryDto);
            //StringContent stringContent = new StringContent(jsonData, Encoding.UTF8, "application/json");
            await client2.GetAsync("http://localhost:5044/api/MenuTables/ChangeMenuTableStatusToTrue?id=" + menuTableId);
